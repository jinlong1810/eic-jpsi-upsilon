...
int plot_dsigma_dt_solid()
{
  ...
  count_dsigma_dt->GetYaxis()->SetTitle("#frac{d#sigma}{dt} (nb/GeV^{2})");
  count_dsigma_dt->GetXaxis()->SetTitle("|t-t_{min}| (GeV^{2})");

  const unsigned nbins_Keq = 4;
  float a_Keqmin[nbins_Keq] = {9.09, 9.29, 9.49, 9.69};
  float a_Keqmax[nbins_Keq] = {9.29, 9.49, 9.69, 9.89};
